...
_GAME_BATTING_INSERT = text("""
    ...
    ON DUPLICATE KEY UPDATE
        ...
        batting_order      = :u_batting_order,
        at_bats            = :u_at_bats,
        runs               = :u_runs,
        hits               = :u_hits,
        doubles_hit        = :u_doubles,
        triples            = :u_triples,
        home_runs          = :u_home_runs,
        inside_the_park_hr = :u_inside_the_park_hr,
        rbi                = :u_rbi,
        walks              = :u_walks,
        strikeouts         = :u_strikeouts,
        stolen_bases       = :u_stolen_bases,
        caught_stealing    = :u_caught_stealing,
        plate_appearances  = :u_plate_appearances,
        hbp                = :u_hbp
""")
...
_GAME_PITCHING_INSERT = text("""
    ...
    VALUES
        (:game_id, :player_id, :team_id, :league_year_id,
         ...
         :pitches_thrown, :balls, :strikes, :hbp, :wildpitches)
    ON DUPLICATE KEY UPDATE
        pitch_appearance_order      = :u_pitch_appearance_order,
        games_started               = :u_games_started,
        win                         = :u_win,
        loss                        = :u_loss,
        save_recorded               = :u_save,
        hold                        = :u_hold,
        blown_save                  = :u_blown_save,
        quality_start               = :u_quality_start,
        innings_pitched_outs        = :u_innings_pitched_outs,
        hits_allowed                = :u_hits_allowed,
        runs_allowed                = :u_runs_allowed,
        earned_runs                 = :u_earned_runs,
        walks                       = :u_walks,
        strikeouts                  = :u_strikeouts,
        home_runs_allowed           = :u_home_runs_allowed,
        inside_the_park_hr_allowed  = :u_inside_the_park_hr_allowed,
        pitches_thrown              = :u_pitches_thrown,
        balls                       = :u_balls,
        strikes                     = :u_strikes,
        hbp                         = :u_hbp,
        wildpitches                 = :u_wildpitches
""")
    ...
    for player_id_str, b in batters.items():
        try:
            ...
            conn.execute(_GAME_BATTING_INSERT, {
                ...
                "hbp":                int(b.get("hbp", 0)),
                "u_position_code":    pos,
                "u_batting_order":    int(b.get("batting_order", 0)),
                "u_at_bats":          int(b.get("at_bats", 0)),
                "u_runs":             int(b.get("runs", 0)),
                "u_hits":             int(b.get("hits", 0)),
                "u_doubles":          int(b.get("doubles", 0)),
                "u_triples":          int(b.get("triples", 0)),
                "u_home_runs":        int(b.get("home_runs", 0)),
                "u_inside_the_park_hr": int(b.get("inside_the_park_hr", 0)),
                "u_rbi":              int(b.get("rbi", 0)),
                "u_walks":            int(b.get("walks", 0)),
                "u_strikeouts":       int(b.get("strikeouts", 0)),
                "u_stolen_bases":     int(b.get("stolen_bases", 0)),
                "u_caught_stealing":  int(b.get("caught_stealing", 0)),
                "u_plate_appearances": int(b.get("plate_appearances", 0)),
                "u_hbp":              int(b.get("hbp", 0)),
            })
            count += 1
        except Exception as e:
    ...
    for player_id_str, p in pitchers.items():
        try:
            conn.execute(_GAME_PITCHING_INSERT, {
                ...
                "u_pitch_appearance_order":    int(p.get("pitch_appearance_order", 0)),
                "u_games_started":             int(p.get("games_started", 0)),
                "u_win":                       int(p.get("win", 0)),
                "u_loss":                      int(p.get("loss", 0)),
                "u_save":                      int(p.get("save", 0)),
                "u_hold":                      int(p.get("hold", 0)),
                "u_blown_save":                int(p.get("blown_save", 0)),
                "u_quality_start":             int(p.get("quality_start", 0)),
                "u_innings_pitched_outs":      int(p.get("innings_pitched_outs", 0)),
                "u_hits_allowed":              int(p.get("hits_allowed", 0)),
                "u_runs_allowed":              int(p.get("runs_allowed", 0)),
                "u_earned_runs":               int(p.get("earned_runs", 0)),
                "u_walks":                     int(p.get("walks", 0)),
                "u_strikeouts":                int(p.get("strikeouts", 0)),
                "u_home_runs_allowed":         int(p.get("home_runs_allowed", 0)),
                "u_inside_the_park_hr_allowed": int(p.get("inside_the_park_hr_allowed", 0)),
                "u_pitches_thrown":            int(p.get("pitches_thrown", 0)),
                "u_balls":                     int(p.get("balls", 0)),
                "u_strikes":                   int(p.get("strikes", 0)),
                "u_hbp":                       int(p.get("hbp", 0)),
                "u_wildpitches":               int(p.get("wildpitches", 0)),
            })
            count += 1
        except Exception as e:
